...
update_env() {
    ...
    echo -e "${BLUE}📝 Updating unified .env file with ngrok URL...${NC}"
    
    # Update unified .env file
    if [ -f "$root_env_file" ]; then
        echo -e "${YELLOW}   Updating root .env file...${NC}"
        cp "$root_env_file" "$root_env_file.backup"
        
        # Remove existing PUBLIC_URL and ngrok URLs
        grep -v '^PUBLIC_URL=' "$root_env_file" | grep -v '^NGROK_' > "$root_env_file.tmp"
        mv "$root_env_file.tmp" "$root_env_file"
        
        # Add new PUBLIC_URL and ngrok URLs
        echo "PUBLIC_URL=\"$ngrok_url\"" >> "$root_env_file"
        echo "" >> "$root_env_file"
        echo "# Auto-generated ngrok URLs (for reference)" >> "$root_env_file"
        echo "NGROK_URL=$ngrok_url" >> "$root_env_file"
        echo "NGROK_WS_URL=$ws_url" >> "$root_env_file"
        echo "NGROK_HTTPS_URL=$ngrok_url" >> "$root_env_file"
        
        echo -e "${GREEN}   ✅ Updated root .env with PUBLIC_URL and ngrok URLs${NC}"
    else
        echo -e "${RED}   ❌ Root .env file not found at $root_env_file${NC}"
        return 1
    fi
    
    echo -e "${GREEN}✅ Updated unified .env file with:${NC}"
    echo -e "   PUBLIC_URL: $ngrok_url"
    echo -e "   HTTPS URL: $ngrok_url"
    echo -e "   WebSocket URL: $ws_url"
    echo -e "   Backup saved as .env.backup"
}
...
if [ $? -eq 0 ] && [ ! -z "$NGROK_URL" ]; then
    ...
    if grep -q "PUBLIC_URL=\"$NGROK_URL\"" "../../.env"; then
        echo -e "${GREEN}✅ Unified .env file updated successfully${NC}"
        ...
        echo -e "${RED}❌ Failed to update unified .env file${NC}"
