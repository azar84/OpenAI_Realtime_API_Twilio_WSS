...
export async function getAllAgentConfigs(): Promise<any[]> {
  const client = await pool.connect();
  try {
    ...
    const configsWithSecondaryLanguages = await Promise.all(
      mainResult.rows.map(async (row) => {
        ...
        if (row.secondary_language_ids && row.secondary_language_ids.length > 0) {
          const secondaryQuery = `
            ...
            ORDER BY name
          `;
          const secondaryResult = await client.query(secondaryQuery, [row.secondary_language_ids]);
        ...
        return {
          ...row,
          secondary_language_codes: secondaryLanguages,
          secondary_language_names: secondaryLanguageNames
        };
      })
    );
...
export async function getAgentConfigById(id: number): Promise<DBAgentConfig | null> {
  const client = await pool.connect();
  try {
...
export async function getPersonalityOptions(): Promise<any[]> {
  const client = await pool.connect();
  try {
    const query = `
...
export async function createAgentConfig(configData: any): Promise<DBAgentConfig> {
  const client = await pool.connect();
  try {
    const query = `
...
export async function updateAgentConfig(id: number, configData: any): Promise<any> {
  const client = await pool.connect();
  try {
...
export async function activateAgentConfig(id: number): Promise<void> {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    
    // First, deactivate all configurations
    await client.query('UPDATE agent_configs SET is_active = false');
...
export async function createPersonalityOption(optionData: any): Promise<any> {
  const client = await pool.connect();
  try {
    const { category, value, sort_order } = optionData;
    const result = await client.query(
      `INSERT INTO personality_options (category, value, description, sort_order, is_active, created_at, updated_at) 
       VALUES ($1, $2, null, $3, true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP) 
       RETURNING *`,
      [category, value, sort_order || 1]
    );
    return result.rows[0];
  } catch (error) {
...
export async function updatePersonalityOption(id: number, optionData: any): Promise<any> {
  const client = await pool.connect();
  try {
    const { category, value, sort_order } = optionData;
    const result = await client.query(
      `UPDATE personality_options 
       ...
       WHERE id = $4 
       RETURNING *`,
      [category, value, sort_order || 1, id]
    );
...
export async function deletePersonalityOption(id: number): Promise<void> {
  const client = await pool.connect();
  try {
    const result = await client.query('DELETE FROM personality_options WHERE id = $1', [id]);
